...
book *create_book(char *title, char *author, int year, char *subject, int book_id) //tworzenie ksiazki do struktury
{
...
list *add_book(list *head) //dodaje ksiazke do listy + sprawdzanie poprawnosci danych
{
    ...
    int check_id = 1;
    int p = 1;
    while (check_title == 1 || atoi(title) != 0) //sprawdzanie unikatowosci imienia
    {
        printf("Podaj tytul:\n");
